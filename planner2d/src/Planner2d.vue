...
import { objectGraph } from './composables/objGraphs/objGraph'
...
function mouseDoubleClick(e: Point) {
  if (settings.value.mode === Mode.Furniture) {
    ...
    floorPlanGraph.addNode(toNextNumber(floorplanProjection.to(e)));
  }
  drawMain();
}
...
function mouseDown(e: Point) {
  ...
  if (settings.value.mode === Mode.Floorplan) {
    if (floorPlanGraph.handleClick(e)) {
      selected = true;
    }
...
function mouseMove(e: Point) {
  ...
  if (settings.value.mode === Mode.Floorplan) {
    if (floorPlanGraph.handleMove(e)) {
      changed = true;
    }
...
function mouseUp(e: Point) {
  if (settings.value.mode === Mode.Floorplan) {
    floorPlanGraph.handleUnclick(e);
...
onMounted(() => {
  ...
  const defaultObjectFloorPlanGraph = {
      count: 0,
      nodes: {},
      edges: {},
      addNode: function (p: Point): number {
      ...
      handleNodeToNodeSnap: function (node: CornerNode, p: Point, extendNode: boolean): boolean {
        ...
        for (const other of Object.values(this.nodes)) {
          ...
          const dist = distance(other.p, floorplanProjection.to(p));
          if (dist < settings.value.nodeExtendSize && (minDist === null || dist < minDist)) {
            minDist = dist;
            node.delta = floorplanProjection.from(other.p);
            node.snap.x = other.id;
            node.snap.y = other.id;
      ...
      handleNodeToEdgeSnap: function (node: CornerNode, p: Point, extendNode: boolean): boolean {
        const clickPos = floorplanProjection.to(p);
        ...
        for (const outEdges of Object.values(this.edges)) {
          for (const edge of Object.values(outEdges)) {
            ...
            if (dist < settings.value.nodeExtendSize && (minDist === null || dist < minDist)) {
              ...
              node.snap.edge = edge;
              node.snap.pos = t;
              node.delta = floorplanProjection.from(proj);
              if (!extendNode) {
                node.p = proj;
              }
            }
      ...
      handleNodeToNeighborSnap: function (node: CornerNode, p: Point, extendNode: boolean, change: boolean): optionalPoint {
        ...
        for (const other of Object.values(this.nodes)) {
          ...
          if (dist.x < settings.value.nodeExtendSize && (minDist.x === null || dist.x < minDist.x) && dist.x <= dist.y) {
            minDist.x = dist.x;
            if (change) {
              node.delta.x = floorplanProjection.from(other.p).x;
              if (!extendNode) {
                node.p.x = other.p.x;
              }
            }
            ...
            minDist.y = dist.y;
            if (change) {
              node.delta.y = floorplanProjection.from(other.p).y;
              if (!extendNode) {
                node.p.y = other.p.y;
              }
            }
      ...
      handleNodeSnap: function (node: CornerNode, p: Point, extendNode: boolean) {
        ...
        // if no snapping happend
        const proj = toNextNumber({
          x: node.p.x + (p.x - node.delta.x) / floorplanProjection.scale,
          y: node.p.y + (p.y - node.delta.y) / floorplanProjection.scale
        });
        if (minDist.x === null) {
          node.snap.x = null;
          if (!extendNode) {
            node.p.x = proj.x;
            node.delta.x = floorplanProjection.from(proj).x;
          } else {
            node.delta.x = p.x;
          }
        }
        if (minDist.y === null) {
          ...
          if (!extendNode) {
            node.p.y = proj.y;
            node.delta.y = floorplanProjection.from(proj).y;
          } else {
            node.delta.y = p.y;
          }
        }
        ...
        for (const node of Object.values(this.nodes)) {
          if (node.remove && node.translate) {
            ...
            if (node.snap.x !== null && node.snap.y !== null && node.snap.x === node.snap.y) {
              ...
              const newId = this.addNode(
                toNextNumber(floorplanProjection.to({
                  x: node.snap.x === null && node.snap.edge === null ? e.x : node.delta.x,
                  y: node.snap.y === null && node.snap.edge === null ? e.y : node.delta.y
                })));
              this.addEdge(node.id, newId);
        ...
        for (const outEdges of Object.values(this.edges)) {
          for (const edge of Object.values(outEdges)) {
            ...
            if ((!node1.remove && node1.translate) || (!node2.remove && node2.translate)) {
              ...
              const other = node1.translate ? node1 : node2;
              const dist = distance(node1.p, node2.p);
              const ul = { x: -floorplanProjection.p.x / floorplanProjection.scale, y: -floorplanProjection.p.y / floorplanProjection.scale };
              const br = floorplanProjection.to({ x: canvas.value.width, y: canvas.value.height });
      ...
      drawExtend: function () {
        for (const node of Object.values(this.nodes)) {
          if (node.extend) {
            const newPos = floorplanProjection.to(node.delta);
            if (node.remove) {
              ctx.value.fillStyle = "red";
              ctx.value.strokeStyle = "red";
            } else {
    ...
    for (const prop in defaultObjectFloorPlanGraph) {
      floorPlanGraph[prop] = defaultObjectFloorPlanGraph[prop]
    }
